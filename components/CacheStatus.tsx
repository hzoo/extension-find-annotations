...
export default function CacheStatus({ className = "" }: CacheStatusProps) {
  ...
  useEffect(() => {
    ...
    const loadingChangeListener = () => {
      if (!loading.value) { // Update when loading finishes
        updateCacheTimestamp();
      }
    };
  ...
  } else if (ageMs < 2592000000) { // Less than a month
  ...
  if (ageMs > 86400000) { // More than a day
    statusColor = 'bg-orange-500';
  } else if (ageMs > 3600000) { // More than an hour
  ...
  return (
    <div className={`text-xs flex items-center ${className}`}>
      <span className={`inline-block w-2 h-2 ${statusColor} rounded-full mr-1`} />
      <span className="text-gray-500" title={timeString}>Cache updated {ageDisplay}</span>
